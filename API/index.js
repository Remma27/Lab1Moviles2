...
app.post('/volcanoes', upload.array('images', 10), async (req, res) => {
    try {
        ...
        const { name, location, elevation } = req.body;
        const images = req.files.map(file => file.buffer);

        // Validate required fields
        if (!name || !location || !elevation || !images || images.length === 0) {
            return res.status(400).json({ error: 'Please provide name, location, elevation, and at least one image' });
        }

        // Validate the data types
        // if (typeof name !== 'string' || typeof location !== 'string' || typeof elevation !== 'number') {
        //     return res.status(400).json({ error: 'Invalid data types for name, location, or elevation' });
        // }
        ...
        const newVolcano = await Volcano.create({ name, location, elevation, images });
...
app.patch('/volcanoes/:id', upload.array('images', 10), async (req, res) => {
    const id = req.params.id;
    const { name, location, elevation, images } = req.body;
    ...
    try {
        ...
        // Validate required fields
        if (!name && !location && !elevation && !images) {
        ...
        if (elevation && typeof elevation !== 'number') {
            return res.status(400).json({ error: 'Invalid data type for elevation' });
        }
        ...
        const updatedVolcano = await Volcano.findByIdAndUpdate(id, { name, location, elevation, images }, { new: true });
        if (updatedVolcano) {
